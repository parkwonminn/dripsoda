...
const getSelectedContinent = () => {
    const selectedValue = continentContainer
        .querySelector(':scope > .continent[selected]')?.dataset.value;
...
const drawSubs = (continent) => {
                ...
                .forEach(region => {
                    ...
                    regionElement.addEventListener('click', e => {
                        ...
                        setSelectedCountry(e.target
                            .parentNode
                            .parentNode
                            .querySelector('.country[data-value]').dataset.value);
    ...
    setSelectedCountry(subContainer
        .querySelector(':scope > .sub:first-of-type > .country').dataset.value);
};
    ...
    xhr.onreadystatechange = () => {
        if (xhr.readyState === XMLHttpRequest.DONE) {
            ...
            if (xhr.status >= 200 && xhr.status < 300) {
                ...
                form['dateFrom'].value = `${dateFromObj.getFullYear()}-${dateFromObj.getMonth() + 1 < 10 ? '0' : ''}${dateFromObj.getMonth() + 1}-${dateFromObj.getDate()}`;
                form['dateTo'].value = `${dateToObj.getFullYear()}-${dateToObj.getMonth() + 1 < 10 ? '0' : ''}${dateToObj.getMonth() + 1}-${dateToObj.getDate()}`;
